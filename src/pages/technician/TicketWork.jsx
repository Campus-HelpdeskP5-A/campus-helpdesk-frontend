  ...
  return (
    <div style={{ maxWidth: 560 }}>
      ...
      <div className="btn-row">
        <button className="btn ghost sm" disabled={busy} onClick={() => handleStatus('progress', 'الحالة اتغيّرت لـ In progress')}>Start work</button>
        <button className="btn ghost sm" disabled={busy} onClick={() => handleStatus('pending', 'الحالة اتغيّرت لـ Pending')}>Waiting</button>
        <button className="btn primary sm" disabled={busy} onClick={handleResolve}>Resolve</button>
      </div>
    </div>
  )
}
